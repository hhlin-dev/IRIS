...
const processFiles = async (files) => {
    ...
    logs.value.push(`[SYSTEM] ${files.length} file(s) selected, ready to upload.`);
};
    ...
    try {
        if (uploadedFiles.value.length > 0) {
            ...
            if (res.errors && res.errors.length > 0) {
                res.errors.forEach(err => logs.value.push(`[WARN] ${err}`));
            }
            if (res.chunks_stored === 0) {
                triggerWarning("No text could be extracted. PDFs may be scanned images or empty.");
            }
        } else {
        ...
        streamChat(
            ...
            actualMode,
            (data) => {
                    // Handle backend-streamed errors
                    if (data.step === 'ERROR') {
                        const errMsg = data.data?.error || 'Unknown server error';
